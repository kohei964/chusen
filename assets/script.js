...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector("#exclude-form button").addEventListener("click", function () {
        const excludeInput = document.querySelector("#exclude-input").value.trim();
        if (!excludeInput) {
            alert("除外リストが空です。何も変更されません。");
            return;
        }
    
        const excludeList = excludeInput.split(",").map(item => item.trim());
        console.log("除外リスト:", excludeList);
    
        if (candidates.length === 0) {
            alert("候補リストが空です。先に抽選箱を作成してください。");
            return;
        }
    
        candidates = candidates.filter(candidate => !excludeList.includes(candidate));
        console.log("除外適用後の候補リスト:", candidates);
